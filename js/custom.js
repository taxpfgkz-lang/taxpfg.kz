(function () {
	...
	/* --- 0. Активный пункт меню по текущей странице -----------------------
	   Разметка статическая — CMS не проставляет class="active" автоматически.
	   В HTML зафиксировано active на «Главная» (copy-paste хрома). Определяем
	   нужный пункт по filename и перезаписываем .active на всех .navigation
	   (основная шапка + sticky-клон). Идемпотентно: toggle вместо add/remove. */
	function initActiveNav() {
		var filename = window.location.pathname.split('/').pop() || 'index.html';

		var servicePages = [
			'services.html', 'accounting.html', 'taxes.html',
			'registration.html', 'accounting-recovery.html', 'consulting.html'
		];

		var activeHref;
		if (filename === 'about.html') {
			activeHref = 'about.html';
		} else if (servicePages.indexOf(filename) !== -1) {
			activeHref = 'services.html';
		} else if (filename === 'contacts.html') {
			activeHref = 'contacts.html';
		} else if (filename === 'index.html' || filename === '') {
			activeHref = 'index.html';
		} else {
			activeHref = null; // 404.html, privacy.html — без подсветки
		}

		document.querySelectorAll('.navigation.clearfix').forEach(function(nav) {
			nav.querySelectorAll(':scope > li').forEach(function(li) {
				var a = li.querySelector(':scope > a');
				var href = a ? a.getAttribute('href') : '';
				li.classList.toggle('active', !!activeHref && href === activeHref);
			});
		});
	}
	...
	function initWhatsAppFloat() {
		...
		a.target = '_blank';
		a.rel = 'noopener';
		a.setAttribute('aria-label', 'Обратная связь');
	...
	function initHeaderContact() {
		...
		wrap.innerHTML =
			...
			'<a class="pfg-hc-btn pfg-hc-wa" href="https://wa.me/' + WA_NUMBER + '" target="_blank" rel="noopener" aria-label="Обратная связь">' + WA_SVG + '</a>';
