...
- ✓ **Phase 2 (2026-06-26):** Токен-фундамент в `:root custom.css` — 4px spacing scale, fluid clamp() type scale, line-height policy, F1 input 16px, text-wrap balance/pretty, контраст AA (TOK-01/02/03, VIS-02, VIS-03); net-new !important=0, floor a11y≥95/axe=0 удержан — validated
- ✓ **Phase 3 (2026-06-26):** Компоненты системно — кнопки primary/secondary/ghost +4 состояния, формы non-color affordance, карточки stretch, nav-состояния, net-new FAQ-аккордеон (native `<details>`) на services+contacts, FT1 footer/F2 consent ≥44px tap, CMP-06 модалки descoped (CMP-01..06, VIS-01); VIS-01 визуальный sign-off Юрия passed; net-new !important=0, js байт-идентичен (VER-04), floor удержан — validated
...
<!-- Phase 2 завершена 2026-06-26: токен-фундамент (spacing/type/color) приземлён в custom.css. -->
<!-- Phase 3 завершена 2026-06-26: компоненты + FAQ net-new + tap-target фиксы (CMP-01..06, VIS-01). -->
